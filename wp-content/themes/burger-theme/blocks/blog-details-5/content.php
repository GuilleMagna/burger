<?php
$content_fields = array(
    'link_blog_details_html' => 'blog-details.html',
    'imagen_assets_images_blog_blog_1_webp' => NAKAMA_THEME_URL . '/assets/images/blog/blog-1.webp',
    'texto_feb' => 'Feb',
    'link_blog_details_html_2' => 'blog-details.html',
    'texto_business' => 'Business',
    'texto_by' => 'By',
    'link_blog_details_html_3' => 'blog-details.html',
    'texto_ellinien_loma' => 'Ellinien Loma',
    'link_blog_details_html_4' => 'blog-details.html',
    'texto_innovative_solutions_for_every_busines' => 'Innovative Solutions for every Business Success.',
    'link_blog_details_html_5' => 'blog-details.html',
    'texto_read_more' => 'Read More',
    'link_blog_details_html_6' => 'blog-details.html',
    'imagen_assets_images_blog_blog_2_webp' => NAKAMA_THEME_URL . '/assets/images/blog/blog-2.webp',
    'texto_feb_2' => 'Feb',
    'link_blog_details_html_7' => 'blog-details.html',
    'texto_business_2' => 'Business',
    'texto_by_2' => 'By',
    'link_blog_details_html_8' => 'blog-details.html',
    'texto_ellinien_loma_2' => 'Ellinien Loma',
    'link_blog_details_html_9' => 'blog-details.html',
    'texto_harnessing_digital_transform_a_roadmap' => 'Harnessing Digital Transform a Roadmap Businesses.',
    'link_blog_details_html_10' => 'blog-details.html',
    'texto_read_more_2' => 'Read More',
    'link_blog_details_html_11' => 'blog-details.html',
    'imagen_assets_images_blog_blog_3_webp' => NAKAMA_THEME_URL . '/assets/images/blog/blog-3.webp',
    'texto_feb_3' => 'Feb',
    'link_blog_details_html_12' => 'blog-details.html',
    'texto_business_3' => 'Business',
    'texto_by_3' => 'By',
    'link_blog_details_html_13' => 'blog-details.html',
    'texto_ellinien_loma_3' => 'Ellinien Loma',
    'link_blog_details_html_14' => 'blog-details.html',
    'texto_mastering_change_management_lessons_fo' => 'Mastering Change Management Lessons for Businesses.',
    'link_blog_details_html_15' => 'blog-details.html',
    'texto_read_more_3' => 'Read More',
    'link_blog_details_html_16' => 'blog-details.html',
    'imagen_assets_images_blog_blog_1_webp_2' => NAKAMA_THEME_URL . '/assets/images/blog/blog-1.webp',
    'texto_feb_4' => 'Feb',
    'link_blog_details_html_17' => 'blog-details.html',
    'texto_business_4' => 'Business',
    'texto_by_4' => 'By',
    'link_blog_details_html_18' => 'blog-details.html',
    'texto_ellinien_loma_4' => 'Ellinien Loma',
    'link_blog_details_html_19' => 'blog-details.html',
    'texto_innovative_solutions_for_every_busines_2' => 'Innovative Solutions for every Business Success.',
    'link_blog_details_html_20' => 'blog-details.html',
    'texto_read_more_4' => 'Read More',
    'link_blog_details_html_21' => 'blog-details.html',
    'imagen_assets_images_blog_blog_2_webp_2' => NAKAMA_THEME_URL . '/assets/images/blog/blog-2.webp',
    'texto_feb_5' => 'Feb',
    'link_blog_details_html_22' => 'blog-details.html',
    'texto_business_5' => 'Business',
    'texto_by_5' => 'By',
    'link_blog_details_html_23' => 'blog-details.html',
    'texto_ellinien_loma_5' => 'Ellinien Loma',
    'link_blog_details_html_24' => 'blog-details.html',
    'texto_harnessing_digital_transform_a_roadmap_2' => 'Harnessing Digital Transform a Roadmap Businesses.',
    'link_blog_details_html_25' => 'blog-details.html',
    'texto_read_more_5' => 'Read More',
    'link_blog_details_html_26' => 'blog-details.html',
    'imagen_assets_images_blog_blog_3_webp_2' => NAKAMA_THEME_URL . '/assets/images/blog/blog-3.webp',
    'texto_feb_6' => 'Feb',
    'link_blog_details_html_27' => 'blog-details.html',
    'texto_business_6' => 'Business',
    'texto_by_6' => 'By',
    'link_blog_details_html_28' => 'blog-details.html',
    'texto_ellinien_loma_6' => 'Ellinien Loma',
    'link_blog_details_html_29' => 'blog-details.html',
    'texto_mastering_change_management_lessons_fo_2' => 'Mastering Change Management Lessons for Businesses.',
    'link_blog_details_html_30' => 'blog-details.html',
    'texto_read_more_6' => 'Read More',
    'texto_search_here' => 'Search here',
    'texto_related_post' => 'Related post',
    'link_blog_details_html_31' => 'blog-details.html',
    'imagen_assets_images_blog_post_1_webp' => NAKAMA_THEME_URL . '/assets/images/blog/post-1.webp',
    'link_blog_details_html_32' => 'blog-details.html',
    'texto_how_to_stay_ahead_of_the_business_curv' => 'How to Stay Ahead of the Business Curve',
    'texto_04_sep_2025' => '04 SEP 2025',
    'link_blog_details_html_33' => 'blog-details.html',
    'imagen_assets_images_blog_post_2_webp' => NAKAMA_THEME_URL . '/assets/images/blog/post-2.webp',
    'link_blog_details_html_34' => 'blog-details.html',
    'texto_how_digital_tools_shaping_the_workforc' => 'How Digital Tools Shaping the Workforce',
    'texto_02_jan_2025' => '02 JAN 2025',
    'link_blog_details_html_35' => 'blog-details.html',
    'imagen_assets_images_blog_post_3_webp' => NAKAMA_THEME_URL . '/assets/images/blog/post-3.webp',
    'link_blog_details_html_36' => 'blog-details.html',
    'texto_how_to_sustainability_into_your_strate' => 'How to Sustainability into your Strategy',
    'texto_24_feb_2025' => '24 FEB 2025',
    'texto_categories' => 'Categories',
    'link_blog_details_html_37' => 'blog-details.html',
    'texto_innovation' => 'Innovation',
    'link_blog_details_html_38' => 'blog-details.html',
    'texto_leadership' => 'Leadership',
    'link_blog_details_html_39' => 'blog-details.html',
    'texto_technology' => 'Technology',
    'link_blog_details_html_40' => 'blog-details.html',
    'texto_marketing' => 'Marketing',
    'link_blog_details_html_41' => 'blog-details.html',
    'texto_management' => 'Management',
    'texto_tags' => 'Tags',
    'link_blog_details_html_42' => 'blog-details.html',
    'texto_growth' => 'Growth',
    'link_blog_details_html_43' => 'blog-details.html',
    'texto_success' => 'Success',
    'link_blog_details_html_44' => 'blog-details.html',
    'texto_innovate' => 'Innovate',
    'link_blog_details_html_45' => 'blog-details.html',
    'texto_lead' => 'Lead',
    'link_blog_details_html_46' => 'blog-details.html',
    'texto_impact' => 'Impact',
    'link_blog_details_html_47' => 'blog-details.html',
    'texto_focus' => 'Focus',
    'link_blog_details_html_48' => 'blog-details.html',
    'texto_tech' => 'Tech',
    'link_blog_details_html_49' => 'blog-details.html',
    'texto_optimize' => 'Optimize',
    'link_blog_details_html_50' => 'blog-details.html',
    'texto_results' => 'Results',
    'link_blog_details_html_51' => 'blog-details.html',
    'texto_drive' => 'Drive',
);

$fields = get_block_content_fields( $content_fields );
extract( $fields, EXTR_SKIP );

?>

<!-- start: Blog Section 5 -->
<section class="tj-blog-section section-gap">
    <div class="container">
    <div class="row row-gap-5">
        <div class="col-lg-8">
        <div class="row row-gap-4">
            <div class="col-md-6">
            <div class="blog-item wow fadeInUp" data-wow-delay=".1s">
                <div class="blog-thumb">
                <a href="<?php echo esc_url( $link_blog_details_html ); ?>"><img src="<?php echo esc_url( $imagen_assets_images_blog_blog_1_webp ); ?>" alt=""></a>
                <div class="blog-date">
                    <span class="date">28</span>
                    <span class="month"><?php echo esc_html( $texto_feb ); ?></span>
                </div>
                </div>
                <div class="blog-content">
                <div class="blog-meta">
                    <span class="categories"><a href="<?php echo esc_url( $link_blog_details_html_2 ); ?>"><?php echo esc_html( $texto_business ); ?></a></span>
                    <span><?php echo esc_html( $texto_by ); ?> <a href="<?php echo esc_url( $link_blog_details_html_3 ); ?>"><?php echo esc_html( $texto_ellinien_loma ); ?></a></span>
                </div>
                <h4 class="title"><a href="<?php echo esc_url( $link_blog_details_html_4 ); ?>"><?php echo esc_html( $texto_innovative_solutions_for_every_busines ); ?></a>
                </h4>
                <a class="text-btn" href="<?php echo esc_url( $link_blog_details_html_5 ); ?>">
                    <span class="btn-text"><span><?php echo esc_html( $texto_read_more ); ?></span></span>
                    <span class="btn-icon"><i class="tji-arrow-right-long"></i></span>
                </a>
                </div>
            </div>
            </div>
            <div class="col-md-6">
            <div class="blog-item wow fadeInUp" data-wow-delay=".1s">
                <div class="blog-thumb">
                <a href="<?php echo esc_url( $link_blog_details_html_6 ); ?>"><img src="<?php echo esc_url( $imagen_assets_images_blog_blog_2_webp ); ?>" alt=""></a>
                <div class="blog-date">
                    <span class="date">28</span>
                    <span class="month"><?php echo esc_html( $texto_feb_2 ); ?></span>
                </div>
                </div>
                <div class="blog-content">
                <div class="blog-meta">
                    <span class="categories"><a href="<?php echo esc_url( $link_blog_details_html_7 ); ?>"><?php echo esc_html( $texto_business_2 ); ?></a></span>
                    <span><?php echo esc_html( $texto_by_2 ); ?> <a href="<?php echo esc_url( $link_blog_details_html_8 ); ?>"><?php echo esc_html( $texto_ellinien_loma_2 ); ?></a></span>
                </div>
                <h4 class="title"><a href="<?php echo esc_url( $link_blog_details_html_9 ); ?>"><?php echo esc_html( $texto_harnessing_digital_transform_a_roadmap ); ?></a>
                </h4>
                <a class="text-btn" href="<?php echo esc_url( $link_blog_details_html_10 ); ?>">
                    <span class="btn-text"><span><?php echo esc_html( $texto_read_more_2 ); ?></span></span>
                    <span class="btn-icon"><i class="tji-arrow-right-long"></i></span>
                </a>
                </div>
            </div>
            </div>
            <div class="col-md-6">
            <div class="blog-item wow fadeInUp" data-wow-delay=".1s">
                <div class="blog-thumb">
                <a href="<?php echo esc_url( $link_blog_details_html_11 ); ?>"><img src="<?php echo esc_url( $imagen_assets_images_blog_blog_3_webp ); ?>" alt=""></a>
                <div class="blog-date">
                    <span class="date">28</span>
                    <span class="month"><?php echo esc_html( $texto_feb_3 ); ?></span>
                </div>
                </div>
                <div class="blog-content">
                <div class="blog-meta">
                    <span class="categories"><a href="<?php echo esc_url( $link_blog_details_html_12 ); ?>"><?php echo esc_html( $texto_business_3 ); ?></a></span>
                    <span><?php echo esc_html( $texto_by_3 ); ?> <a href="<?php echo esc_url( $link_blog_details_html_13 ); ?>"><?php echo esc_html( $texto_ellinien_loma_3 ); ?></a></span>
                </div>
                <h4 class="title"><a href="<?php echo esc_url( $link_blog_details_html_14 ); ?>"><?php echo esc_html( $texto_mastering_change_management_lessons_fo ); ?></a>
                </h4>
                <a class="text-btn" href="<?php echo esc_url( $link_blog_details_html_15 ); ?>">
                    <span class="btn-text"><span><?php echo esc_html( $texto_read_more_3 ); ?></span></span>
                    <span class="btn-icon"><i class="tji-arrow-right-long"></i></span>
                </a>
                </div>
            </div>
            </div>
            <div class="col-md-6">
            <div class="blog-item wow fadeInUp" data-wow-delay=".1s">
                <div class="blog-thumb">
                <a href="<?php echo esc_url( $link_blog_details_html_16 ); ?>"><img src="<?php echo esc_url( $imagen_assets_images_blog_blog_1_webp_2 ); ?>" alt=""></a>
                <div class="blog-date">
                    <span class="date">28</span>
                    <span class="month"><?php echo esc_html( $texto_feb_4 ); ?></span>
                </div>
                </div>
                <div class="blog-content">
                <div class="blog-meta">
                    <span class="categories"><a href="<?php echo esc_url( $link_blog_details_html_17 ); ?>"><?php echo esc_html( $texto_business_4 ); ?></a></span>
                    <span><?php echo esc_html( $texto_by_4 ); ?> <a href="<?php echo esc_url( $link_blog_details_html_18 ); ?>"><?php echo esc_html( $texto_ellinien_loma_4 ); ?></a></span>
                </div>
                <h4 class="title"><a href="<?php echo esc_url( $link_blog_details_html_19 ); ?>"><?php echo esc_html( $texto_innovative_solutions_for_every_busines_2 ); ?></a>
                </h4>
                <a class="text-btn" href="<?php echo esc_url( $link_blog_details_html_20 ); ?>">
                    <span class="btn-text"><span><?php echo esc_html( $texto_read_more_4 ); ?></span></span>
                    <span class="btn-icon"><i class="tji-arrow-right-long"></i></span>
                </a>
                </div>
            </div>
            </div>
            <div class="col-md-6">
            <div class="blog-item wow fadeInUp" data-wow-delay=".1s">
                <div class="blog-thumb">
                <a href="<?php echo esc_url( $link_blog_details_html_21 ); ?>"><img src="<?php echo esc_url( $imagen_assets_images_blog_blog_2_webp_2 ); ?>" alt=""></a>
                <div class="blog-date">
                    <span class="date">28</span>
                    <span class="month"><?php echo esc_html( $texto_feb_5 ); ?></span>
                </div>
                </div>
                <div class="blog-content">
                <div class="blog-meta">
                    <span class="categories"><a href="<?php echo esc_url( $link_blog_details_html_22 ); ?>"><?php echo esc_html( $texto_business_5 ); ?></a></span>
                    <span><?php echo esc_html( $texto_by_5 ); ?> <a href="<?php echo esc_url( $link_blog_details_html_23 ); ?>"><?php echo esc_html( $texto_ellinien_loma_5 ); ?></a></span>
                </div>
                <h4 class="title"><a href="<?php echo esc_url( $link_blog_details_html_24 ); ?>"><?php echo esc_html( $texto_harnessing_digital_transform_a_roadmap_2 ); ?></a>
                </h4>
                <a class="text-btn" href="<?php echo esc_url( $link_blog_details_html_25 ); ?>">
                    <span class="btn-text"><span><?php echo esc_html( $texto_read_more_5 ); ?></span></span>
                    <span class="btn-icon"><i class="tji-arrow-right-long"></i></span>
                </a>
                </div>
            </div>
            </div>
            <div class="col-md-6">
            <div class="blog-item wow fadeInUp" data-wow-delay=".1s">
                <div class="blog-thumb">
                <a href="<?php echo esc_url( $link_blog_details_html_26 ); ?>"><img src="<?php echo esc_url( $imagen_assets_images_blog_blog_3_webp_2 ); ?>" alt=""></a>
                <div class="blog-date">
                    <span class="date">28</span>
                    <span class="month"><?php echo esc_html( $texto_feb_6 ); ?></span>
                </div>
                </div>
                <div class="blog-content">
                <div class="blog-meta">
                    <span class="categories"><a href="<?php echo esc_url( $link_blog_details_html_27 ); ?>"><?php echo esc_html( $texto_business_6 ); ?></a></span>
                    <span><?php echo esc_html( $texto_by_6 ); ?> <a href="<?php echo esc_url( $link_blog_details_html_28 ); ?>"><?php echo esc_html( $texto_ellinien_loma_6 ); ?></a></span>
                </div>
                <h4 class="title"><a href="<?php echo esc_url( $link_blog_details_html_29 ); ?>"><?php echo esc_html( $texto_mastering_change_management_lessons_fo_2 ); ?></a>
                </h4>
                <a class="text-btn" href="<?php echo esc_url( $link_blog_details_html_30 ); ?>">
                    <span class="btn-text"><span><?php echo esc_html( $texto_read_more_6 ); ?></span></span>
                    <span class="btn-icon"><i class="tji-arrow-right-long"></i></span>
                </a>
                </div>
            </div>
            </div>
        </div>
        <!-- post pagination -->
        <div class="tj-pagination d-flex justify-content-center">
            <ul>
            <li>
                <span aria-current="page" class="page-numbers current">01</span>
            </li>
            <li>
                <a class="page-numbers" href="#">02</a>
            </li>
            <li>
                <a class="next page-numbers" href="#"><i class="tji-arrow-right-long"></i></a>
            </li>
            </ul>
        </div>
        </div>
        <div class="col-lg-4">
        <div class="tj-main-sidebar p-0 slidebar-stickiy">
            <div class="tj-sidebar-widget widget-search wow fadeInUp" data-wow-delay=".1s">
            <h4 class="widget-title"><?php echo esc_html( $texto_search_here ); ?></h4>
            <div class="search-box">
                <form action="#">
                <input type="search" name="search" id="searchTwo" placeholder="Search here">
                <button type="submit" value="search">
                    <i class="tji-search"></i>
                </button>
                </form>
            </div>
            </div>
            <div class="tj-sidebar-widget tj-recent-posts wow fadeInUp" data-wow-delay=".3s">
            <h4 class="widget-title"><?php echo esc_html( $texto_related_post ); ?></h4>
            <ul>
                <li>
                <div class="post-thumb">
                    <a href="<?php echo esc_url( $link_blog_details_html_31 ); ?>"> <img src="<?php echo esc_url( $imagen_assets_images_blog_post_1_webp ); ?>" alt="Blog"></a>
                </div>
                <div class="post-content">
                    <h6 class="post-title">
                    <a href="<?php echo esc_url( $link_blog_details_html_32 ); ?>"><?php echo esc_html( $texto_how_to_stay_ahead_of_the_business_curv ); ?></a>
                    </h6>
                    <div class="blog-meta">
                    <ul>
                        <li><?php echo esc_html( $texto_04_sep_2025 ); ?></li>
                    </ul>
                    </div>
                </div>
                </li>
                <li>
                <div class="post-thumb">
                    <a href="<?php echo esc_url( $link_blog_details_html_33 ); ?>"> <img src="<?php echo esc_url( $imagen_assets_images_blog_post_2_webp ); ?>" alt="Blog"></a>
                </div>
                <div class="post-content">
                    <h6 class="post-title">
                    <a href="<?php echo esc_url( $link_blog_details_html_34 ); ?>"><?php echo esc_html( $texto_how_digital_tools_shaping_the_workforc ); ?></a>
                    </h6>
                    <div class="blog-meta">
                    <ul>
                        <li><?php echo esc_html( $texto_02_jan_2025 ); ?></li>
                    </ul>
                    </div>
                </div>
                </li>
                <li>
                <div class="post-thumb">
                    <a href="<?php echo esc_url( $link_blog_details_html_35 ); ?>"> <img src="<?php echo esc_url( $imagen_assets_images_blog_post_3_webp ); ?>" alt="Blog"></a>
                </div>
                <div class="post-content">
                    <h6 class="post-title">
                    <a href="<?php echo esc_url( $link_blog_details_html_36 ); ?>"><?php echo esc_html( $texto_how_to_sustainability_into_your_strate ); ?></a>
                    </h6>
                    <div class="blog-meta">
                    <ul>
                        <li><?php echo esc_html( $texto_24_feb_2025 ); ?></li>
                    </ul>
                    </div>
                </div>
                </li>
            </ul>
            </div>
            <div class="tj-sidebar-widget widget-categories wow fadeInUp" data-wow-delay=".5s">
            <h4 class="widget-title"><?php echo esc_html( $texto_categories ); ?></h4>
            <ul>
                <li><a href="<?php echo esc_url( $link_blog_details_html_37 ); ?>"><?php echo esc_html( $texto_innovation ); ?><span class="number">(03)</span></a></li>
                <li><a href="<?php echo esc_url( $link_blog_details_html_38 ); ?>"><?php echo esc_html( $texto_leadership ); ?><span class="number">(02)</span></a></li>
                <li><a href="<?php echo esc_url( $link_blog_details_html_39 ); ?>"><?php echo esc_html( $texto_technology ); ?><span class="number">(03)</span></a></li>
                <li><a href="<?php echo esc_url( $link_blog_details_html_40 ); ?>"><?php echo esc_html( $texto_marketing ); ?><span class="number">(06)</span></a></li>
                <li><a href="<?php echo esc_url( $link_blog_details_html_41 ); ?>"><?php echo esc_html( $texto_management ); ?><span class="number">(04)</span></a></li>
            </ul>
            </div>
            <div class="tj-sidebar-widget widget-tag-cloud wow fadeInUp" data-wow-delay=".7s">
            <h4 class="widget-title"><?php echo esc_html( $texto_tags ); ?></h4>
            <nav>
                <div class="tagcloud">
                <a href="<?php echo esc_url( $link_blog_details_html_42 ); ?>"><?php echo esc_html( $texto_growth ); ?></a>
                <a href="<?php echo esc_url( $link_blog_details_html_43 ); ?>"><?php echo esc_html( $texto_success ); ?></a>
                <a href="<?php echo esc_url( $link_blog_details_html_44 ); ?>"><?php echo esc_html( $texto_innovate ); ?></a>
                <a href="<?php echo esc_url( $link_blog_details_html_45 ); ?>"><?php echo esc_html( $texto_lead ); ?></a>
                <a href="<?php echo esc_url( $link_blog_details_html_46 ); ?>"><?php echo esc_html( $texto_impact ); ?></a>
                <a href="<?php echo esc_url( $link_blog_details_html_47 ); ?>"><?php echo esc_html( $texto_focus ); ?></a>
                <a href="<?php echo esc_url( $link_blog_details_html_48 ); ?>"><?php echo esc_html( $texto_tech ); ?></a>
                <a href="<?php echo esc_url( $link_blog_details_html_49 ); ?>"><?php echo esc_html( $texto_optimize ); ?></a>
                <a href="<?php echo esc_url( $link_blog_details_html_50 ); ?>"><?php echo esc_html( $texto_results ); ?></a>
                <a href="<?php echo esc_url( $link_blog_details_html_51 ); ?>"><?php echo esc_html( $texto_drive ); ?></a>
                </div>
            </nav>
            </div>
        </div>
        </div>
    </div>
    </div>

</section>
<!-- end: Blog Section 5 -->
